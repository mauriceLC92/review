...
func TestOpenPostgresStoreFailsToOpenAConnectionToPostgres(t *testing.T) {
	t.Parallel()

	_, err := review.OpenPostgresStore("some-connection-string")
	if err == nil {
		t.Fatal("expected error opening connection to postgres but instead got nil")
	}
}
